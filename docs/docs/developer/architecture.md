...
Fetches template JSON from the configured **online template repo** (the same repo URL as More → Repo URL). The app's runtime template data actually comes from the catalog database (`CatalogRepository`); the resolver is used by the **template creator** page, which always reads the online JSON (`index.json` + `data/**/*.json`) instead of the DB.

```mermaid
classDiagram
    class TemplateResolver {
        +Future~TemplateIndex~ loadIndex({String? baseUrl})
        +Future~InheritedContent~ resolveExtendsChain(List~String~ extendsPaths, {String? baseUrl})
    }

    class InheritedContent {
        +List~InheritedItem~ items
        +List~InheritedPart~ parts
    ...
    TemplateResolver --> InheritedContent
    InheritedContent --> InheritedItem
    InheritedContent --> InheritedPart
```

- `loadIndex(baseUrl:)` fetches `index.json` from the online repo (falling back to bundled assets when offline).
- `resolveExtendsChain(paths, baseUrl:)` walks the full `extends` chain of each selected extends path and returns every inherited part/item tagged with its **origin** file — mirroring `build_catalog.py`'s override/`remove` semantics.
- Used by `template_creator_page.dart` for the make/model autocomplete, the computed extends, and the read-only inherited parts/maintenance lists.
...
- Three files: `templates/i18n/en.json` (English), `templates/i18n/es.json` (Spanish), and `templates/i18n/et.json` (Estonian). Accessible in the app via a symlink at `mobile/i18n/` (listed as a separate asset in `pubspec.yaml` since Flutter doesn't recursively bundle symlinked subdirectories on Linux).
...
| `catalogServiceProvider` | `Provider<CatalogService>` | Catalog download/refresh/import service |
| `catalogRepositoryProvider` | `Provider<CatalogRepository>` | Reads the active catalog DB (index, best match, DTCs) |
| `templateSourceProvider` | `NotifierProvider<TemplateSourceConfig>` | Online template repo URL + version (More → Repo URL) |
| `catalogSourcesProvider` | `NotifierProvider<CatalogSourcesState>` | Active catalog source (builtin / online / local) |
| `onlineTemplateBaseUrlProvider` | `FutureProvider<String?>` | Resolved raw base URL of the template repo |
| `onlineTemplateIndexProvider` | `FutureProvider<TemplateIndex>` | `index.json` fetched from the online repo |
