...
class _VehicleDetailPageState extends ConsumerState<VehicleDetailPage> {
  ...
  void _updateOdometer(BuildContext context, String vehicleId, WidgetRef ref) {
    karterShowModalBottomSheet(
      context: context,
      isScrollControlled: true,
      builder: (ctx) => OdometerDialog(
        current:
            ref.read(vehicleProvider(vehicleId)).value?.currentOdometer ??
  ...
  Widget build(BuildContext context) {
    ...
    return vehicleAsync.when(
      data: (vehicle) {
        ...
        final isKm = vehicle.currentOdometer.unit == DistanceUnit.kilometers;
                  ...
                  child: FloatingActionButton.small(
                    ...
                    onPressed: () {
                      ...
                      showAddDocumentModal(
                        ...
                        onSaved: () {
                          ref.invalidate(
                            vehicleDocumentsProvider(widget.vehicleId),
                          );
                        },
                      );
                    },
      ...
      child: Column(
        ...
        children: [
          ...
          ListTile(
            ...
            title: Text(l.partsTitle),
            trailing: const Icon(Icons.chevron_right),
            onTap: () => context.push('/vehicle/$vehicleId/parts'),
          ),
          ListTile(
            ...
            trailing: const Icon(Icons.chevron_right),
            onTap: () =>
                context.push('/vehicle/$vehicleId/maintenance/settings'),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return intervalsAsync.when(
      data: (intervals) {
        ...
        final intervalData =
            enabled.map((i) => _IntervalData.compute(i, distanceKm, l)).toList()
              ..sort((a, b) {
                if (a.isDue != b.isDue) {
                  return a.isDue ? -1 : 1;
                }
                  ...
                  children: [
                    ...
                    if (interval.parts.isNotEmpty)
                      Padding(
                        ...
                        child: IntervalPartsView(parts: interval.parts),
                      ),
...
class _FabStaggeredEntryState extends State<_FabStaggeredEntry>
  ...
  void initState() {
    super.initState();
    _ctrl = AnimationController(duration: Durations.medium4, vsync: this);
  ...
  void didUpdateWidget(_FabStaggeredEntry oldWidget) {
    ...
    if (widget.visible != oldWidget.visible) {
      if (widget.visible) {
        ...
        Future.delayed(Duration(milliseconds: 50 * (2 - widget.index)), () {
...
class _IntervalData {
  ...
  static _IntervalData compute(
    ...
    final kmSinceReset =
        interval.lastResetKm > 0 && distanceKm >= interval.lastResetKm
    ...
    final kmProgress = interval.kmInterval < 999999 && kmSinceReset > 0
        ...
        : 0.0;
    final timeProgress =
        interval.monthsInterval != null && interval.lastResetDate != null
